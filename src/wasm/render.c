// Spectrogram render kernel: grid of magnitude floats -> RGBA pixels.
//
// Mirrors the JS renderInto in src/client/analysis/spectrogram.ts:
//   db = mag > floor_mag ? 20 * log10(mag / mag_ref) : floor_db
//   db = clamp(db, floor_db, ceil_db)
//   t  = (db - floor_db) / (ceil_db - floor_db)
//   idx = round(clamp(t * 255, 0, 255))
//   out at flipped row = lut[idx]
//
// The output buffer is RGBA, width=cols, height=num_bins, row-major,
// with row 0 = highest frequency (image-top). Source grid is column-major:
...
static inline v128_t v_log10_ps(v128_t x) {
    ...
    // exponent_unbiased = ((i >> 23) & 0xff) - 127
    ...
    // Mantissa with exponent forced to 127: m = 1.frac in [1, 2)
    v128_t m_bits = wasm_v128_and(i, wasm_i32x4_splat(0x007fffff));
    m_bits = wasm_v128_or(m_bits, wasm_i32x4_splat(0x3f800000));
    v128_t m = m_bits; // reinterpret as f32x4

    // log2(m) ≈ c0 + c1 m + c2 m² + c3 m³ + c4 m⁴ + c5 m⁵ (Horner, low→high).
    ...
    v128_t log10_2 = wasm_f32x4_splat(0.30102999566398f); // log10(2)
...
void render_grid_to_rgba(
    ...
    float mag_ref,
    float floor_mag,
    const uint8_t* lut, // 256 * 4 bytes RGBA
    uint8_t* out_rgba   // cols * num_bins * 4 bytes
) {
    const float range = ceil_db - floor_db;
    const float inv_range = 1.0f / range;
    const float inv_mag_ref = 1.0f / mag_ref;

    v128_t v_floor_mag = wasm_f32x4_splat(floor_mag);
    v128_t v_inv_mag_ref = wasm_f32x4_splat(inv_mag_ref);
    v128_t v_twenty = wasm_f32x4_splat(20.0f);
    ...
    for (int col = 0; col < cols; col++) {
        ...
        for (; b + 4 <= num_bins; b += 4) {
            v128_t mag = wasm_v128_load(src + b);
            // mag > floor_mag ? mag : floor_mag (so log10 stays defined)
            v128_t gated = wasm_f32x4_max(mag, v_floor_mag);
            v128_t ratio = wasm_f32x4_mul(gated, v_inv_mag_ref);
            v128_t log10r = v_log10_ps(ratio);
            v128_t db = wasm_f32x4_mul(v_twenty, log10r);
            // If mag <= floor_mag, force db = floor_db (matches the JS branch)
            v128_t mask = wasm_f32x4_gt(mag, v_floor_mag);
            ...
            // t = (db - floor_db) * inv_range; idx = clamp(round(t*255), 0, 255)
            v128_t t = wasm_f32x4_mul(wasm_f32x4_sub(db, v_floor_db), v_inv_range);
            v128_t scaled = wasm_f32x4_mul(t, v_255);
            // round-to-nearest via add 0.5 + truncate (values are in [0, 255])
            ...
            // Scalar gather + scatter to flipped row
            for (int j = 0; j < 4; j++) {
        ...
        for (; b < num_bins; b++) {
            ...
            float db;
            if (mag > floor_mag) {
                db = 20.0f * log10f(mag * inv_mag_ref);
            } else {
                db = floor_db;
            }
